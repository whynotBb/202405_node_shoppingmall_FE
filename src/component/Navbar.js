		...
		<div>
				...
				<div className="display-space-between mobile-search-box w-100">
					<div className="search display-space-between w-100">
						<div>
							...
							<input type="text" placeholder="제품검색123" onKeyPress={onCheckEnter} />
						</div>
			...
			<div className="nav-header">
				...
				<div>
					<div className="display-flex">
						...
						<div onClick={() => navigate("/cart")} className="nav-icon">
							...
							{!isMobile && (
								<span style={{ cursor: "pointer" }}>
									쇼핑백<i className="shopping_bag_badge">{cartItemQty || 0}</i>
								</span>
							)}
						</div>
						...
						<div onClick={() => navigate("/event")} className="nav-icon">
							<FontAwesomeIcon icon={faCalendarCheck} />
							{!isMobile && <span style={{ cursor: "pointer" }}>출석체크</span>}
						</div>
						{isMobile && (
			...
			<div className="nav-menu-area">
				<ul className="menu">
					{menuList.map((menu, index) => (
						<li key={index}>
							<span onClick={() => navigate(`/?category=${menu.toLowerCase()}`)}>{menu}</span>
						</li>
					))}
				</ul>
