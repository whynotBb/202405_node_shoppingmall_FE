...
const Event = () => {
	...
	useEffect(() => {
		...
		if (addPointList && addPointList.length > 0) {
			const lastDate = addPointList[addPointList.length - 1].date.slice(0, 10);
			if (lastDate === today && !isChallenge) {
				setIsChallenge(true);
			}
		}
	}, [addPointList]);
	...
	return (
		<Container className="roulette_wrap">
			...
			<div className="roulette_box">
				...
				{/* {isChallenge && <div className="roulette_dim">{resultMsg ? resultMsg : "오늘은 이미 이벤트에 참여하셨습니다!"}</div>} */}
			</div>
